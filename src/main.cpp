...
const auto heart_beat_delay = 4s;
...
try{
    ...
    if(!ports.contains("motor_controller"))
    {
        throw std::runtime_error("motor_controller not connected.");
    }
    ...
    rmq.listen_to_queue(1,"motor_ctrl");
    cub::Serial_command cm{cub::Serial_connector{ports["motor_controller"]}};
    std::jthread sending_sig_th{ [&cm]()
    {
        for(;;)
        {
            cm.run("a",run_delay);
            std::this_thread::sleep_for(heart_beat_delay);
        }
    }};
    ...
    std::jthread serial_access_th{ [&ports]()
    {
        for(;;)
        {
            if(access(ports["motor_controller"].c_str(), R_OK|W_OK) != 0)
            {
                throw std::runtime_error("serial disconnected.");
            }
            std::this_thread::sleep_for(port_check_delay);
        }
    }};
